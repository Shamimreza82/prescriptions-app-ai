  ...
  return (
    <div className="min-h-screen bg-[#f7f9fb] dark:bg-gray-950">
      <div className="max-w-[1600px] mx-auto p-8">
            ...
            <div className="space-y-4">
              {medFields.map((field, i) => (
                <div key={field.id} className="grid grid-cols-12 gap-4 items-start">
                  ...
                  <div className="col-span-6 md:col-span-1 flex items-end gap-1.5 pt-1.5">
                    <input {...register(`medicines.${i}.instructions`)} placeholder="Note" className="w-full bg-gray-50 dark:bg-gray-800/50 border border-gray-200/60 dark:border-gray-700/60 rounded-xl p-3.5 text-sm focus:ring-2 focus:ring-teal-500/30 focus:outline-none hidden md:block" title="Instructions" />
                    {medFields.length > 1 && (
                    ...
                    return (
                      <tr key={field.id} className="bg-white dark:bg-gray-900 hover:bg-teal-50/30 dark:hover:bg-teal-950/20 transition-colors group">
                        <td className="px-6 py-5">
                          ...
                          <p className="text-xs text-gray-400">{m.instructions || 'No instructions'}</p>
                        </td>
                        <td className="px-6 py-5">
        ...
        <aside className="col-span-12 lg:col-span-5 xl:col-span-4">
          <div className="sticky top-8 space-y-6">
            ...
            <div className="bg-white dark:bg-gray-900 rounded-lg shadow-2xl overflow-hidden min-h-[842px] relative transform origin-top transition-transform duration-500 border border-gray-100 dark:border-gray-800">
              {/* Letterhead */}
                      ...
                      medFields.map((field, i) => {
                        ...
                        return (
                          <div key={field.id} className="relative pl-2 border-l-2 border-teal-300/50">
                            ...
                            {m.instructions && <p className="text-gray-400 italic text-[9px]">{m.instructions}</p>}
                          </div>
                        );
                      })
                    )}
      ...
      <div className="fixed bottom-8 left-1/2 -translate-x-1/2 z-50 flex items-center space-x-4 bg-white/90 dark:bg-gray-950/90 backdrop-blur-xl rounded-full shadow-2xl p-2 border border-gray-200 dark:border-gray-800">
        <button type="button" disabled className="text-gray-600 dark:text-gray-300 px-6 py-3 flex items-center gap-2 hover:scale-105 transition-transform active:scale-95 text-[11px] font-bold uppercase tracking-wider opacity-60 cursor-not-allowed">
