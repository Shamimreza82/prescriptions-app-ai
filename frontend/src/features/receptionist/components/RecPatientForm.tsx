...
import { useCreateRecPatient, useUpdateRecPatient } from '../hooks';
...
interface RecPatientFormProps {
  onSuccess?: () => void;
  defaultValues?: FormData;
  patientId?: string;
}

export const RecPatientForm = ({ onSuccess, defaultValues, patientId }: RecPatientFormProps) => {
  const create = useCreateRecPatient();
  const update = useUpdateRecPatient();
  ...
  const onSubmit = async (data: FormData) => {
    try {
      if (patientId) {
        await update.mutateAsync({ id: patientId, data });
      } else {
