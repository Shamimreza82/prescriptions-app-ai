...
import {
  ...
  Shield, UserCog, CreditCard, PersonStanding, ClipboardList, UserRound, Crown, Settings, ChevronDown, BarChart3, Database, SearchCheck,
} from 'lucide-react';
...
const mrMenu = [
  ...
  { href: '/dashboard/mr/audit', label: 'Audit', icon: SearchCheck },
...
export const Sidebar = () => {
  ...
  const renderMenuItem = (item: MenuItem) => {
    ...
    return (
      <div key={`${item.href}-${item.label}`} title={isCollapsed ? item.label : undefined}>
        <Link
          href={item.href!}
          className={cn(
    ...
    <>
      ...
      <aside
        className={cn(
          ...
          'lg:translate-x-0',
          isCollapsed && !mobileOpen ? 'w-[72px]' : 'w-[260px]'
        )}
      >
        {/* Brand */}
        <div className={cn('p-4 border-b border-gray-100 dark:border-gray-800/50 flex items-center', isCollapsed ? 'justify-center' : 'justify-between')}>
          {!isCollapsed && (
        ...
        {/* Footer */}
        <div className={cn('p-4 border-t border-gray-100 dark:border-gray-800/50 space-y-2', isCollapsed ? 'flex flex-col items-center' : '')}>
          <button
            onClick={toggle}
            className={cn(
          ...
          <button
            ...
            className={cn(
              ...
              isCollapsed ? 'justify-center p-3' : 'w-full px-3.5 py-2.5'
            )}
